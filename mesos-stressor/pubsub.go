...
//
// utils to use pub/sub system
//
//
func SubscribeTaskUpdate(taskID string) *mesosproto.Event {
	tf := func(v interface{}) bool {
		ev, ok := v.(*mesosproto.Event)
		if !ok {
			return false
		}

		if ev.GetType() != mesosproto.Event_UPDATE {
			return false
		}
	...
	ev := <-sub
	return ev.(*mesosproto.Event)
}

func SubscribeOffer() *mesosproto.Offer {
	tf := func(v interface{}) bool {
		ev, ok := v.(*mesosproto.Event)
		if !ok {
			return false
		...
		if ev.GetType() != mesosproto.Event_OFFERS {
			return false
		}

		return len(ev.Offers.Offers) > 0
	}
	...
	ev := <-sub
	return ev.(*mesosproto.Event).Offers.Offers[0]
}
